...
const ProblemStatements = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios.get(`${server1}/hackathon/problem-statement`); 
  ...
  if (isLoading) {
    return (
      <div className="bg-gray-900 min-h-screen flex items-center justify-center">
        <h2 className="text-2xl text-white">Loading problem statements...</h2>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-4 max-w-screen-xl">
        {filteredStatements.map((statement, index) => (
          ...
          >
            ...
            <div className="mt-11 flex items-center mb-4">
              {statement.icon && React.createElement(statement.icon, { className: "text-3xl" })}
              <h3 className="text-xl font-semibold ml-4 text-white">
                {statement.title}
              </h3>
            </div>
